...
cNetwork::cNetwork()
{
    ...
    // set up the network socket we use to send/recv server packets
    ...
    ULONG nonblock = 1;
    if (ioctlsocket(m_sSocket, FIONBIO, &nonblock) == SOCKET_ERROR) {
        MessageBox(NULL, "Failed to set socket to non-blocking!", "Error", MB_OK);
    }
    

	FD_ZERO(&m_masterfds);
	FD_SET(m_sSocket, &m_masterfds);

	m_socketTimeout.tv_sec = 0;
	m_socketTimeout.tv_usec = TIMEOUT_PACKET_POLL_MS*1000;
...
#define SERVER_IP_CHARS_MAX 64
	...
	m_siLoginServer.clientXorGen = NULL;
    m_siLoginServer.m_dwConnectAttempts = 0;
...
void cNetwork::Reset()
{
	m_Interface->OutputConsoleString("Resetting network state...");
...
static DWORD checksumPacket(cPacket *packet, ChecksumXorGenerator * xorGen)
{
	...
	if (xorGen == NULL) {
		...
		xor = xorGen->get(header->m_dwSequence);
	}
	DWORD xorVal = (header->m_dwFlags & kEncryptedChecksum) ? xor : 0;
...
void cNetwork::SendLoginPacket(){
	m_Interface->OutputConsoleString("Sending login packet...");
...
void cNetwork::Run()
{
	...
	while (!m_bQuit)
	{
		// if we get into the network Run() loop before reset has happened
		if (m_siLoginServer.m_dwConnectAttempts == 0) {
			m_Interface->OutputConsoleString("Initiating first connection attempt...");
            Connect();
		}
		...
		if (!(m_siLoginServer.m_dwFlags & SF_CONNECTED))
		{
			...
			if ((m_siLoginServer.m_dwLastConnectAttempt + TIMEOUT_CONNECT_MS) < GetTickCount())
			{
				m_Interface->OutputConsoleString("Timed out after %d ms.", TIMEOUT_CONNECT_MS);
		...
		if (m_dwNextConnectionTimeout > 0 && m_dwNextConnectionTimeout < GetTickCount()) {
            m_Interface->OutputConsoleString("%d: Timed out trying to connect or enter game..", GetTickCount());
			MessageBox(NULL, "Timed out trying to connect or enter game", "Network Error", MB_OK);
			m_dwNextConnectionTimeout = 0;
			Connect();
		}
        ...
        else if (FD_ISSET(m_sSocket, &m_readfds)) {
            // there's a packet waiting (we don't need to check which socket since there's only 1 in the set)
			ReceivePacket();
        }
        else {
            // timed out
        ...
        // we have an active world server connection
        if (m_pActiveWorld != NULL) {
...
void cNetwork::ReceivePacket(){
	// receive a packet after we know one is waiting because of the select call in Run()
	
	// the Packet must be freed before this function returns
	cPacket *Packet = new cPacket();
	BYTE bRawData[NETWORK_PACKET_BUF_SIZE];
	SOCKADDR Target;

	int siSize = sizeof(sockaddr_in);
	int iRawSize = recvfrom(m_sSocket, (char *)bRawData, NETWORK_PACKET_BUF_SIZE, NULL, &Target, &siSize);
...
void cNetwork::ProcessPacket(cPacket *Packet, stServerInfo *Server)
{
	...
	if (dwFlags & kConnectRequest)
	{
        ...
        m_Interface->OutputConsoleString("%d: Sending connect response...", GetTickCount());
		...
		// Next we should get a CharacterList message
		// set up a timeout in case we don't get it
		m_dwNextConnectionTimeout = TIMEOUT_CONNECT_MS + GetTickCount();
		m_Interface->OutputConsoleString("%d: Waiting for character list...", GetTickCount());
	}
...
void cNetwork::ProcessMessage(cMessage *Msg, stServerInfo *Server)
{
	...
	{
	case GameMessages::InventoryRemoveObject:
	{
		//destroy object
		m_World->DeleteObject(Msg->ReadDWORD());
		break;
	}
	case GameMessages::SetStackSize:
	{
		//adjust stack size
	...
	case GameMessages::PlayerKilled:
	{
		//player kill
	...
	case GameMessages::CreatureMessage:
	{
		//chat window message
		char *text = Msg->ReadString();
		char *senderName = Msg->ReadString();
	...
	case GameMessages::PrivateUpdatePropertyInt:
	{
		//set Character DWORD
		BYTE sequence = Msg->ReadByte();
	...
	case GameMessages::PublicUpdatePropertyInt:
	{
		//set Object DWORD
		BYTE sequence = Msg->ReadByte();
	...
	case GameMessages::PrivateUpdatePropertyInt64:
	{
		//set Character QWORD
		BYTE sequence = Msg->ReadByte();
		DWORD key = Msg->ReadDWORD();
	...
	case GameMessages::PrivateUpdatePropertyBool:
	{
	...
	case GameMessages::PublicUpdatePropertyBool:
	{
		//set Object Boolean
		BYTE sequence = Msg->ReadByte();
		DWORD object = Msg->ReadDWORD();
	...
	case GameMessages::PublicUpdatePropertyString:
	{
		//set Object String
		BYTE sequence = Msg->ReadByte();
		DWORD key = Msg->ReadDWORD();
	...
	case GameMessages::PublicUpdatePropertyDataID:
	{
		//set Object Resource
		BYTE sequence = Msg->ReadByte();
		DWORD object = Msg->ReadDWORD();
	...
	case GameMessages::PrivateUpdatePropertyInstanceID:
	{
		//set Character Link
		BYTE sequence = Msg->ReadByte();
		DWORD key = Msg->ReadDWORD();
	...
	case GameMessages::PublicUpdateInstanceId:
	{
		//set Object Link
		BYTE sequence = Msg->ReadByte();
		DWORD object = Msg->ReadDWORD();
	...
	case GameMessages::PrivateUpdatePosition:
	{
	...
	case GameMessages::PrivateUpdateSkill:
	{
	...
	case GameMessages::PrivateUpdateAttribute:
	{
	...
	case GameMessages::PrivateUpdateVital:
	{
	...
	case GameMessages::PrivateUpdateAttribute2ndLevel:
	{
	...
	case GameMessages::PositionAndMovement:
	{
		//lifestone recall
	...
	case GameMessages::ObjDescEvent:
	{
		//change model
		DWORD object = Msg->ReadDWORD();
		...
		woThis->ParseMessageObjDesc(Msg);
		WORD modelSequenceType = Msg->ReadWORD();
		WORD modelSequence = Msg->ReadWORD();
		break;
	}
	case GameMessages::CharacterCreateResponse:
	{
	...
	case GameMessages::CharacterLogOff:
	{
		m_Interface->OutputConsoleString("Character logged of.");
		//End 3D Mode
	...
	case GameMessages::CharacterDelete:
	{
		// TODO: Char Deletion
		break;
	}
	case GameMessages::CharacterList:
	{
		//Character List
		stCharList CharList;
	...
	case GameMessages::CharacterError:
	{
		//Character Error
	...
	case GameMessages::PlayerCreate:
	{
		DWORD object = Msg->ReadDWORD();
	...
	case GameMessages::UpdatePosition:
	{
		//set position/motion
		DWORD object = Msg->ReadDWORD();
		...
		//    break;
		//}
		tpObj->ParseMessageUpdatePosition(Msg);
		break;
	}
	case GameMessages::ParentEvent:
	{
	...
	case GameMessages::Motion:
	{
		...
		if (!woThis)
			return;
		woThis->ParseMessageMotion(Msg);

		break;
	}
	case GameMessages::VectorUpdate:
	{
		// TODO: jumping...
	...
	case GameMessages::PlayerTeleport:
	{
		// TODO: enter portal mode

		//figure this out at some point
		break;
	}
	case GameMessages::PlayEffect:
	{
	...
	case GameMessages::CharacterEnterWorldServerReady:
	{
	...
	case GameMessages::ServerMessage:
	{
		//display message
		char *text = Msg->ReadString();
	...
	case GameMessages::DDD_Interrogation:
	{
		SendDDDInterrogationResponse();
		break;
	}
	case GameMessages::DDD_EndDDD:
	{
		SendDDDEndMessage();
		break;
	}
	case GameMessages::GameEvent:
	{
		DWORD character = Msg->ReadDWORD();
		...
		case GameEvents::AllegianceUpdate:
		{
		...
		case GameEvents::FriendsListUpdate:
		{
			//Friends list Update
		...
		case GameEvents::CharacterTitle:
		{
			//TitleList
		...
		case GameEvents::QueryAgeResponse:
		{
			//age command result
		...
		case GameEvents::AllegianceAllegianceUpdateDone:
		{
			//Allegiance info 
		...
		case GameEvents::MagicRemoveMultipleEnchantments: case GameEvents::MagicDispelMultipleEnchantments:
		{
...
void cNetwork::SendEnterWorldRequest(DWORD GUID)
{
	...
	m_dwNextConnectionTimeout = GetTickCount() + TIMEOUT_ENTER_GAME_MS;
}
